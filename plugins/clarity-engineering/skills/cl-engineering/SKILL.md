...
When Review begins, the default posture is to make completed work reviewable: inspect git status, commit intended changes using the author's or codebase's existing commit patterns, push the branch, and raise or update a PR when the repository workflow supports PRs. Review is the stage that raises the PR. If a PR already exists, update it instead of creating a duplicate. When committing, discover and follow the author's or codebase's existing commit patterns from git history, docs, and commit tooling; do not invent a new convention. When preparing a PR, discover and follow the repository-local PR template if one exists (for example conventional `.github` template locations). Do not use or mention machine-specific absolute template paths in portable framework output. Include validation evidence from tests, builds, e2e, manual QA, screenshots, logs, or CI in the PR description/review summary.
