...
When Review begins, the default posture is to work from the existing review surface: usually a draft PR or pushed branch. Discover the PR first, update it when needed, and create one only if no suitable review surface exists yet. Include validation evidence and intent drift notes in the PR description.
